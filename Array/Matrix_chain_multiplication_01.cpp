...
int matrix_mult(int arr[10][10], vector<int>arr_ele, int x,int y)
{
	...
	if(x==y)
		return 0;//arr[][] != 0 retrun a[][] 
	else
	{
		if(x != (y-1))
			s1 = matrix_mult(arr, arr_ele, x, y-1) + (arr_ele[x]*arr_ele[y]*arr_ele[y+1]);
		if(y+1 < arr_ele.size())
			s2 = matrix_mult(arr, arr_ele, x+1, y) + (arr_ele[x]*arr_ele[x+1]*arr_ele[y+1]);
		if(s1<s2)
		{
